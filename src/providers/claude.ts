...
import { anthropicSource } from "../models/sources/anthropic.js";
...
export const claudeProvider: ProviderAdapter = {
  ...
  async detect(): Promise<ProviderDetection> {
    ...
    return { available: result.status === 0, detail: result.status === 0 ? result.stdout.trim() : "claude binary unavailable" };
  },
  async status(): Promise<ProviderStatus> {
    const detection = await this.detect({} as ProviderContext);
    return { provider: "claude", availability: detection.available ? "available" : "unavailable", detail: detection.detail, checkedAt: new Date().toISOString() };
  },
  async capabilities(): Promise<ProviderCapabilities> {
    return { provider: "claude", canLaunch: true, structuredOutput: true, worktree: true, cloudHosted: true };
  },
  async discoverModels(): Promise<ModelCatalogEntry[]> {
    return (await anthropicSource.discover()).entries;
  },
...
function parseClaudeStream(stdout: string): { status?: "completed" | "failed"; summary: string } {
  ...
  if (completed) return { status: "completed", summary: summary || "claude completed" };
  if (failed) return { status: "failed", summary: summary || "claude failed" };
  return { summary };
}
